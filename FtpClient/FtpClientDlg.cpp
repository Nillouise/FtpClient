...
//
// FTP 的主动连接，要先发监听端口到服务器，然后 当客户端发list命令时，服务器才会连上这个监听端口。
// 另外由于一开始我在虚拟机上做这东西，ip端口没能穿越好（ftp协商的时候，数据不是写在ip报头里面的，不能用UPnP 穿越），要在真机上做这东西才行的。一开始我就想到上面那个bug，但因为在虚拟机里修，就因为ip没有穿透，修不好了。
